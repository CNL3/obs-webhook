...
# Runtime memory to track the most recent room name
...
def trigger_obs():
    ...
    latest_room_name = room_name  # ✅ store latest room for other scripts
...
def get_latest_room():
    ...
    return jsonify({"room_name": None})

@app.route("/form")
def form_foster():
    room = generate_room_name()
    return f""" ... """  # (no changes made here)
